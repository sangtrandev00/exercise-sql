...
alter table sach
add constraint chk_soTrang check (soTrang >5),
add constraint chk_banSao check (banSao > 1),
...
add constraint checkNgayTra check(Date(ngaytra) >= Date(ngayMuon)),
add constraint checkSoLuong check ( soLuongSach <=3 );
